...
import { fetchGenresTV, fetchTVPopular } from '@/apis/tv-list';
...
export default function TVPopularPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const tvResponse = await fetchTVPopular(page != null ? parseInt(page) : 1, filteredParams.toString());
  ...
  return (
    <div className="flex flex-col min-h-screen font-geist-mono">
      <main className="container mx-auto py-10">
        <h2 className="mb-5 text-2xl font-bold ">Popular TV Series</h2>
